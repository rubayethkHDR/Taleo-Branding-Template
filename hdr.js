(function () {
  ...
  var C = {
    init: function () {
      ...
      defaultOptions.cookieName =
        defaultOptions.ns + "_" + defaultOptions.cookieName;
      C.POPUP.init();
    },
  };
  ...
  C.POPUP = {
    ...
    show: function () {
      ...
      label = label.replace(
        /\[cookie_accept_url\](.+?)\[\/cookie_accept_url\]/g,
        '<a href="' +
          url +
          '" style="' +
          C.UTIL.createCSSMarkup(defaultOptions.cssStyles.aCookieLaw, ";") +
          '" >$1</a>'
      );

      div.setAttribute("id", defaultOptions.ns + "-" + "cookie-law");
      div.style.cssText = C.UTIL.createCSSMarkup(
        defaultOptions.cssStyles.divCookieLaw,
        ";"
      );
    ...
    createCloseBtn: function () {
      ...
      if (this.isMediaQueriesSupported()) {
        var mq = window.matchMedia("(max-width: 768px)"),
          setMQ = function (mq) {
            if (mq.matches) {
              link.style.cssText = C.UTIL.createCSSMarkup(
                defaultOptions.cssStyles.aCookieLawHideMobile,
                ";"
              );
            } else {
              link.style.cssText = C.UTIL.createCSSMarkup(
                defaultOptions.cssStyles.aCookieLawHide,
                ";"
              );
            }
          };
  ...
  C.UTIL = {
    ...
    detectLang: function () {
      var urlParamLang = (location.search.split("lang=")[1] || "").split(
        "&"
      )[0];
    ...
    translate: function (key) {
      return defaultOptions.translations.hasOwnProperty(defaultOptions.lang) &&
        defaultOptions.translations[defaultOptions.lang].hasOwnProperty(key)
        ? defaultOptions.translations[defaultOptions.lang][key]
        : defaultOptions.translations[defaultOptions.fallbackLang][key];
    },
  };
